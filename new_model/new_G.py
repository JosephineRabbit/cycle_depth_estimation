...
class depth_block(nn.Module):
    def __init__(self,in_c):
        ...
        for i in range(4):
            ...
            self.depth_out.append(nn.Sequential(nn.Conv2d(int(in_c/2),1,1,1),nn.Tanh()
                                                ))
        ...
        self.conv = nn.Sequential(nn.ConvTranspose2d(int(in_c*2),int(in_c/2),4,2,padding=1),
                                     nn.LeakyReLU(0.02),
        ...
        self.depconv = nn.Sequential(nn.Conv2d(int(in_c/2),1,1),#nn.BatchNorm2d(1),
                                     nn.Tanh()
                                     )
...
class ResNetLW(nn.Module):
    ...
    def __init__(self, block, layers):
       ...
       # self.do = nn.Dropout(p=0.5)
        ...
        self.p_ims1d2_outl1_dimred_ = convbnrelu(2048,512,1)
       # self.mflow_conv_g1_pool = self._make_crp(512, 512, 4)
        self.psp4 = self._make_psp(512, 512)
        self.mflow_conv_g1_b3_joint_varout_dimred = conv1x1(512,256, bias=False)
        self.p_ims1d2_outl2_dimred_= convbnrelu(1024, 256, 1)
        self.adapt_stage2_b2_joint_varout_dimred = conv1x1(256, 256, bias=False)
       # self.mflow_conv_g2_pool = self._make_crp(256, 256, 4)
        self.psp3 = self._make_psp(256,256)
        self.CAT3 = ATBlock(256,256)
...
class Generator(nn.Module):
    def __init__(self,channels):
        ...
        self.Conv2d(channels[0],channels[1]/2,4,2,1)
        self.BatchNorm
...
if __name__ == '__main__':
    ...
    ori_dict = torch.load('/home/dut-ai/Documents/light-weight-refinenet-master/models/rf_lw50_img.pth')
    segd8 = segd(n_cls=40,up_scale=3,init_channel=256).cuda()
    ...
    for i,name in enumerate(ori_dict):
        ...
        m += 1
    print(n,m)
    torch.save(G_dict, './my_res50_model.pth')
    print(net)
