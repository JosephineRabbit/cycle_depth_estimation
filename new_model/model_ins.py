...
from my_rf_lw import rf_lw101,segd,seg_gan_loss
...
class BaseModel():
    ...
    def save_networks(self, epoch):
        for name in self.model_names:
            save_dir = './checkpoints/2_13_new_ins'
...
class Seg_Depth(BaseModel):
   ...
   #     self.net_G = nn.DataParallel(self.net_G)
        self.net_G.load_state_dict(torch.load(
            '/home/dut-ai/Documents/temp/code/pytorch-CycleGAN-and-pix2pix/new_model/my_ins_res101_model.pth' ))
        self.net_G = nn.DataParallel(self.net_G)


        self.net_seg8 = segd(up_scale=3,n_cls=28,init_channel=512 )
        self.net_seg8 = nn.DataParallel(init_net(self.net_seg8))

    #    self.net_seg8.load_state_dict(torch.load('./checkpoints/2_12_new/iter_4000_net_seg8.pth'))

        self.net_seg4 = segd(up_scale=2, n_cls=28)
        self.net_seg4 = nn.DataParallel(init_net(self.net_seg4))
    #    self.net_seg4.load_state_dict(torch.load('/home/dut-ai/Documents/temp/code/pytorch-CycleGAN-and-pix2pix/new_model/checkpoints/2_12_new/iter_4000_net_seg4.pth'))


        self.net_seg2 = segd(up_scale=1, n_cls=28)
        self.net_seg2 = nn.DataParallel(init_net(self.net_seg2))
    #    self.net_seg2.load_state_dict(torch.load('/home/dut-ai/Documents/temp/code/pytorch-CycleGAN-and-pix2pix/new_model/checkpoints/2_12_new/iter_4000_net_seg2.pth'))
    ...
    def backward_DIS(self):
        ...
        D_seg8_loss = seg8_loss_syn + 0.05*seg8_loss_real
        ...
        seg4_loss_syn = self.criterionGAN(self.up2(syn_seg4),
                                           self.syn_seg_l, target_is_real=True)
        D_seg4_loss = seg4_loss_syn + 0.05*seg4_loss_real
        ...
        seg2_loss_syn = self.criterionGAN(self.up2(syn_seg2),
                                           self.syn_seg_l, target_is_real=True)
        D_seg2_loss = seg2_loss_syn +0.05* seg2_loss_real
        ...
        seg2_0_loss_syn = self.criterionGAN(self.up2(syn_seg2_0),
                                             self.syn_seg_l, target_is_real=True)
        D_seg2_0_loss = seg2_0_loss_syn+0.05*seg2_0_loss_real
    ...
    def backward_G(self,train_or_test):
        ...
        real_dep_outs,real_pred_d,real_features = self.net_G(self.real_img,type='real')
       ...
       # print(real_seg8.shape,real_seg4.shape,real_seg2.shape,real_seg2_0.shape)
        ...
        G_real_loss =  seg8_loss_real + seg4_loss_real + 5*seg2_0_loss_real + seg2_loss_real

        print('dep_real', float(dep_loss_real), 'deg_real_8', float(seg8_loss_real), 'deg_real_4', float(seg4_loss_real),
              'deg_real_2', float(seg2_loss_real), 'dep_real_2_0', float(seg2_0_loss_real))

        self.real_dep_pre = self.up2(real_pred_d).detach()

        syn_dep_outs, syn_pred_d, syn_features = self.net_G(self.syn_img,type='syn')
...
#        segs_loss_syn = []
        ...
        if train_or_test=='train':
            ...
            print('dep_syn',float(dep_loss_syn),'deg_syn_8',float(seg8_loss_syn),'deg_syn_4',float(seg4_loss_syn),'deg_syn_2',float(seg2_loss_syn),'dep_syn_2_0',float(seg2_0_loss_syn))


        G_syn_loss =seg8_loss_syn+seg4_loss_syn+5*seg2_0_loss_syn+seg2_loss_syn+ 3*dep_loss_syn
