...
class Seg_Depth(BaseModel):
    ...
    def initialize(self,opt):
        ...
        if self.is_Train:
            self.model_names = ['G_1', 'G_2', 'Dis_en','Dep_de',
                               'Seg_de'#,'DIS'
                                ]
        else:  # during test time, only load Gs
            self.model_names = ['G_1', 'G_2','Dep_de',
                               'Seg_de']
        self.net_Dis_en = networks2.Discriminator_seg().cuda()
      ...
      #      'iter_60000_net_Dis_en.pth'))
       ...
       # self.net_Dep_de = init_net(self.net_Dep_de)
        ...
        self.optimizer_Dep = torch.optim.Adam(self.net_Dep_de.parameters(),
                                            lr=opt.lr/2, betas=(opt.beta1, 0.999))
        self.optimizer_D = torch.optim.SGD(self.net_Dis_en.parameters(),
                                              lr=opt.lr/3)
    ...
    def backward_D(self):
        ...
        pre_s = self.net_Dis_en(syn_features1)
        self.loss_D_syn = self.criterionGAN(pre_s, True)
        pre_r = self.net_Dis_en(real_features1)
        self.loss_D_real = self.criterionGAN(pre_r, False)
        loss_grad = self.calc_gradient_penalty(netD =self.net_Dis_en
                                               ,real_data=real_features1,
                                               fake_data=syn_features1)
        return self.loss_D_syn + self.loss_D_real+loss_grad
    ...
    def backward_G_1(self):
        self.set_requires_grad([self.net_Seg_de,self.net_G_2,self.net_Dis_en,self.net_Dep_de], False)
    ...
    def backward_G_2(self):
        self.set_requires_grad([self.net_Seg_de, self.net_G_1, self.net_Dis_en, self.net_Dep_de], False)
        self.real_features1 = self.net_G_2(self.real_img, 'R')

        pre_R = self.net_Dis_en(self.real_features1)
        self.loss_G2_dis = self.criterionGAN(pre_R, True)
    ...
    def optimize_parameters(self,train_or_test):
        ...
        if (train_or_test == 'train'):
            self.set_requires_grad([self.net_Dis_en], True)  # self.net_DIS], True)
            self.set_requires_grad([self.net_G_1, self.net_G_2,
                                    self.net_Seg_de, self.net_Dep_de], False)
        ...
        self.set_requires_grad([self.net_Seg_de, self.net_G_2,self.net_Dis_en], False)
