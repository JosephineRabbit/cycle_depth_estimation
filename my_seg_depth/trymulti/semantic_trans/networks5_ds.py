...
class G_side(nn.Module):
    def __init__(self,side_c,df_c,f_size):
        ...
        self.conv = nn.Sequential(nn.Conv2d(2*df_c,df_c,3,1,padding=1),nn.LeakyReLU(0.02),nn.BatchNorm2d(df_c))
    ...
    #    print('d_f',d_featuers.shape)
        ...
        out = torch.cat([d_featuers,s_f],1)
        out = self.conv(out)
        return out

class depth_block(nn.Module):
    def __init__(self,in_c,f_size):
    ...
    def forward(self,in_f):
        ...
        return  F,dep_o,torch.cat([ds_e,dep_1],1)

class seg_block(nn.Module):
    def __init__(self,in_c,out_c):
        super(seg_block, self).__init__()
        self.deconv = torch.nn.ConvTranspose2d(in_c, out_c, 4, 2, 1)
...
class R_dep(nn.Module):
    ...
    def forward(self,s_features,d_feature):
        ...
        out1 = self.AT[1](s_features[2], dep_f1)
    #    out1.backward()
        print('---------------')
        ...
        out2 = self.AT[2](s_features[1],dep_f2)
        ...
        return (S_e160,S_e320,s_e640),(Seg_160,Seg_320,seg640),(Dep_160,Dep_320,Dep_640)
...
if __name__ == '__main__':
    ...
    SeD,Seg,Dep = R_D(features,syn_features1)
