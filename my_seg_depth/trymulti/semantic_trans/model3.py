...
from my_seg_depth.trymulti.semantic_trans import networks2
from my_seg_depth.trymulti.semantic_trans.networks2 import init_net, init_weights,GramMatrix
...
class Seg_Depth(BaseModel):
    ...
    def initialize(self,opt):
        ...
        self.net_DIS = networks2.Discriminator(repeat_num=4).cuda()
        ...
        self.net_G_1.load_state_dict(torch.load(
         './checkpoints/2scale/iter_18000_net_G_1.pth'))
        print('1')
        ...
        './checkpoints/2scale/iter_18000_net_G_2.pth'))
        print('2')
      ...
      #  self.net_Seg_de = init_net(self.net_Seg_de)
        ...
        self.net_Seg_de.load_state_dict(torch.load(
            './checkpoints/2scale/iter_18000_net_Seg_de.pth'))
      ...
      #  self.net_Dep_de = init_net(self.net_Dep_de)
        ...
        self.net_Dep_de.load_state_dict(torch.load(
            './checkpoints/2scale/iter_18000_net_Dep_de.pth'))
        self.net_R_D = networks2.R_dep()
        #self.net_R_D = nn.DataParallel(self.net_R_D).cuda()
        self.net_R_D = init_net(self.net_R_D)
        #self.net_R_D.load_state_dict(torch.load(
        #    './checkpoints/2scale/iter_6000_net_R_D.pth'
        #))

        self.net_Dis_80 = networks2.Discriminator(curr_dim=1, repeat_num=3).cuda()
        self.net_Dis_80 = init_net(self.net_Dis_80)

        self.net_Dis_160 = networks2.Discriminator(curr_dim=1,repeat_num=3).cuda()
        self.net_Dis_160 = init_net(self.net_Dis_160)

        self.net_Dis_320 = networks2.Discriminator(curr_dim=1,repeat_num=3).cuda()
    ...
    #    self.optimizer_D2.zero_grad()
     #   self.optimizer_D3.zero_grad()
        syn_features1,syn_Features = self.net_G_1(self.syn_img)
        ...
        real_features1 = self.net_G_2(self.real_img)[0].detach()
  ...
  #      self.loss_D_real2 = self.criterionGAN(pre_r2, False)
   #     self.loss_D_real3 = self.criterionGAN(pre_r3, False)
    ...
    def backward_R_D(self,train_or_test):
        ...
        real_style_F,real_dep_ref,real_seg_80,real_seg_160,real_dep_160,real_dep_320 = self.net_R_D(
            self.detach_list(self.real_Features)
                                                 ,self.real_features1.detach())

        D_real = self.net_DIS(torch.cat([real_dep_ref,self.real_seg_l.detach().unsqueeze(1).float()],1))

        D_real_160=self.net_Dis_160(real_dep_160)
        D_real_320 = self.net_Dis_320(real_dep_320)

        D_real_loss = 5*(self.criterionGAN(D_real,False)+self.criterionGAN(D_real_160,False)+self.criterionGAN(D_real_320,False))\
                      +self.criterionSeg(real_seg_80,self.real_seg_l)\
                      +self.criterionSeg(real_seg_160, self.real_seg_l)
        ...
        real_style = []
       # for i in range(3):
       #     real_style.append(GramMatrix(real_style_F[i+1].detach()))

        self.optimizer_R_D.zero_grad()
        syn_style_F,syn_dep_ref,syn_seg_80,syn_seg_160,syn_dep_160,syn_dep_320  = \
            self.net_R_D(self.detach_list(self.syn_Features),
                                                self.syn_features1.detach())
        self.syn_dep_ref =syn_dep_ref.detach()
        self.syn_dep_160 = syn_dep_160.detach()
        self.syn_dep_320 = syn_dep_320.detach()
        syn_style=[]
       # for i in range(3):
       #     syn_style.append(GramMatrix(syn_style_F[i+1]))
       # loss_style_0=self.criterionStyle(syn_style[0],real_style[0])
        #self.loss_style_0 = loss_style_0.detach()
        #loss_style_1 = self.criterionStyle(syn_style[1], real_style[1])
        #self.loss_style_1 = loss_style_1.detach()
        #loss_style_2 = self.criterionStyle(syn_style[2], real_style[2])
        #self.loss_style_2 = loss_style_2.detach()

        D_syn_loss = self.criterionSeg(syn_seg_80, self.syn_seg_l)\
                      + self.criterionSeg(syn_seg_160,self.syn_seg_l)\
                     + self.criterionDep(syn_dep_ref,self.syn_dep_l)+self.criterionDep(syn_dep_160,nn.UpsamplingNearest2d(scale_factor=0.25)
        (self.syn_dep_l))+self.criterionDep(syn_dep_320,nn.UpsamplingNearest2d(scale_factor=0.5)(self.syn_dep_l))
        loss_dep_ref = self.criterionDep(syn_dep_ref,self.syn_dep_l)
        self.loss_dep_ref = loss_dep_ref.detach()

       # dis_512_real = self.net_Dis_512(syn_style_F[0])

        return D_syn_loss#+10**-30*(loss_style_0+loss_style_1+loss_style_2)
    ...
    def backward_G_1(self):
        ...
        self.syn_features1,self.syn_Features = self.net_G_1(self.syn_img)
    ...
    def backward_G_2(self):
        ...
        self.real_features1,self.real_Features = self.net_G_2(self.real_img  )
