...
def _pspTrans( num_input_features):
        num_output_features = int(num_input_features/8)
        trans = nn.ModuleList()
        trans.append(nn.BatchNorm2d(num_input_features))
        trans.append(nn.ReLU(inplace=True))
        trans.append(nn.Conv2d(num_input_features, num_output_features,
                                          kernel_size=1, stride=1, bias=False))
        trans.append(nn.Conv2d(num_input_features, num_output_features,
                                            kernel_size=1, stride=1,dilation=1, bias=False))
        trans.append(nn.Conv2d(num_input_features, num_output_features,
                                            kernel_size=2, stride=1, dilation=2,padding=1, bias=False))
        trans.append(nn.Conv2d(num_input_features, num_output_features,
                                            kernel_size=3, stride=1, padding=2, dilation=2, bias=False))
        trans.append(nn.AvgPool2d(kernel_size=2, stride=2))
        return trans
...
#            print(i)
            if i<3:
                input = self.PSP[i][0](input)
                input = self.PSP[i][1](input)
                input = torch.cat([self.PSP[i][2](input),
                                   self.PSP[i][3](input),
                                   self.PSP[i][4](input),
                                   self.PSP[i][5](input)], 1)
                input = self.PSP[i][6](input)
