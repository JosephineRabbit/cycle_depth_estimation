...
class _UNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, layers=4, norm='batch', activation='PReLU', drop_rate=0, add_noise=False,
                 gpu_ids=[0],weight=0.1):
        ...
        self.deconv4_syn = _DecoderUpBlock(ngf*(4+4), ngf*8, ngf*2, norm_layer, nonlinearity, use_bias)
        self.deconv3_syn = _DecoderUpBlock(ngf*(2+2)+nc_syn, ngf*4, ngf, norm_layer, nonlinearity, use_bias)
        self.deconv2_syn = _DecoderUpBlock(ngf*(1+1)+nc_syn, ngf*2, int(ngf/2), norm_layer, nonlinearity, use_bias)

        self.output4_syn = _OutputBlock(ngf*(4+4), nc_syn, 3, use_bias)
        self.output3_syn = _OutputBlock(ngf*(2+2)+nc_syn, nc_syn, 3, use_bias)
        self.output2_syn = _OutputBlock(ngf*(1+1)+nc_syn, nc_syn, 3, use_bias)
        self.output1_syn = _OutputBlock(int(ngf/2)+nc_syn, nc_syn, 7, use_bias)


        self.deconv4_real = _DecoderUpBlock(ngf*(4+4), ngf*8, ngf*2, norm_layer, nonlinearity, use_bias)
        self.deconv3_real = _DecoderUpBlock(ngf*(2+2)+nc_real, ngf*4, ngf, norm_layer, nonlinearity, use_bias)
        self.deconv2_real = _DecoderUpBlock(ngf*(1+1)+nc_real, ngf*2, int(ngf/2), norm_layer, nonlinearity, use_bias)

        self.output4_real = _OutputBlock(ngf*(4+4), nc_real, 3, use_bias)
        self.output3_real = _OutputBlock(ngf*(2+2)+nc_real, nc_real, 3, use_bias)
        self.output2_real = _OutputBlock(ngf*(1+1)+nc_real, nc_real, 3, use_bias)
        self.output1_real = _OutputBlock(int(ngf/2)+nc_real, nc_real, 7, use_bias)

        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')

    def forward(self, input,syn_or_real):
        conv1 = self.pool(self.conv1(input))
        ...
        #     model = getattr(self, 'up'+str(i))
        #     print(model)
        #     center_out = model.forward(torch.cat([center_out, middle[self.layers-5-i]], 1))  #256,1/8
        if(syn_or_real=='syn'):
            deconv4 = self.deconv4_syn.forward(torch.cat([center_out, conv3 * self.weight], 1))
            output4 = self.output4_syn.forward(torch.cat([center_out, conv3 * self.weight], 1))
            # result.append(output4)
            deconv3 = self.deconv3_syn.forward(torch.cat([deconv4, conv2 * self.weight * 0.5, self.upsample(output4)], 1))
            output3 = self.output3_syn.forward(torch.cat([deconv4, conv2 * self.weight * 0.5, self.upsample(output4)], 1))
            # result.append(output3)
            deconv2 = self.deconv2_syn.forward(torch.cat([deconv3, conv1 * self.weight * 0.1, self.upsample(output3)], 1))
            output2 = self.output2_syn.forward(torch.cat([deconv3, conv1 * self.weight * 0.1, self.upsample(output3)], 1))
            # result.append(output2)
            output1 = self.output1_syn.forward(torch.cat([deconv2, self.upsample(output2)], 1))
            result.append(output1)
        else:
            deconv4 = self.deconv4_real.forward(torch.cat([center_out, conv3 * self.weight], 1))
            output4 = self.output4_real.forward(torch.cat([center_out, conv3 * self.weight], 1))
            # result.append(output4)
            deconv3 = self.deconv3_real.forward(
                torch.cat([deconv4, conv2 * self.weight * 0.5, self.upsample(output4)], 1))
            output3 = self.output3_real.forward(
                torch.cat([deconv4, conv2 * self.weight * 0.5, self.upsample(output4)], 1))
            # result.append(output3)
            deconv2 = self.deconv2_real.forward(
                torch.cat([deconv3, conv1 * self.weight * 0.1, self.upsample(output3)], 1))
            output2 = self.output2_real.forward(
                torch.cat([deconv3, conv1 * self.weight * 0.1, self.upsample(output3)], 1))
            # result.append(output2)
            output1 = self.output1_real.forward(torch.cat([deconv2, self.upsample(output2)], 1))
            result.append(output1)
        return result
